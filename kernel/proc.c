...
found:
  ...
  p->pagetable_each_process = kvminit_newpgtbl();
  ...
  kvmmap(p->pagetable_each_process, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);
  p->kstack = va;
...
freeproc(struct proc *p)
{
  ...
  // 释放进程的内核栈
  void *kstack_pa = (void *)kvmpa(p->pagetable_each_process, p->kstack);
  // printf("trace: free kstack %p\n", kstack_pa);
  kfree(kstack_pa);
  p->kstack = 0;

  kvm_free_kernelpgtbl(p->pagetable_each_process);
  p->pagetable_each_process = 0;
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        w_satp(MAKE_SATP(p->pagetable_each_process));
        sfence_vma();

        swtch(&c->context, &p->context);

        kvminithart(); //没有进程的时候，切换回全局的kernel pagetable
